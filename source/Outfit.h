...
inline const std::string &Outfit::Name() const        { return name; }
inline const std::string &Outfit::Category() const    { return category; }
inline const std::string &Outfit::Description() const { return description; }
// Get the image to display in the outfitter when buying this item.
inline const Sprite *Outfit::Thumbnail() const   { return thumbnail; }

inline double Outfit::Get(const std::string &attribute) const
{
...
inline const std::vector<std::pair<Body, int>> &Outfit::FlareSprites() const { return flareSprites; }
inline const std::map<const Sound *, int> &Outfit::FlareSounds()  const { return flareSounds; }
// Get the afterburner effect, if any.
inline const std::map<const Effect *, int> &Outfit::AfterburnerEffects() const { return afterburnerEffects; }
// Get the sprite this outfit uses when dumped into space.
